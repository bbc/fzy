...
def filter_matching_paths(
    ...
    if path_to_string(_path) in paths:
        ...
        new_dict = {}
        for key, value in obj.items():
            matching_value = filter_matching_paths(value, paths, _path + [key])
        ...
        new_list = []
        for i, value in enumerate(obj):
            matching_value = filter_matching_paths(value, paths, _path + [i])
            if matching_value is not NoMatch:
...
def fuzzy_filter(obj: Any, search: str, case_sensitive: bool = False) -> Any:
    ...
    selected_paths = set()
    for path, value in iter_flat(obj):
        if matcher.search(f"{path}:={value}"):
            selected_paths.add(path)

    # Return the subset of the document containing those paths.
    return filter_matching_paths(obj, selected_paths)
